...
int main(){
    ...
    for(int i = 0; i < N; i++){
        ...
        if(pid == 0){
            ...
            if(i == 0){
                num = 0;
                printf("- Processo figlio di indice i = %d, INIZIO PIPELINE\n", i);
            }
            else{
                //Lettura struttura dati proveniente da pipe e controllo esito
                nr = read(piped[i-1][0], &num, sizeof(num));
                if(nr != sizeof(num)){
            ...
            nw = write(piped[i][1], &num, sizeof(num));
            if(nw != sizeof(num)){
    ...
    //Chiusura delle pipe in scrittura e lettura (eccetto l'ultima in lettura)
    for(int k = 0; k < N; k++){
        close(piped[k][1]);
        if(k != N-1){
            close(piped[k][0]);
    ...
    //Lettura struttura dati proveniente da pipe e controllo esito
    nr = read(piped[N-1][0], &num, sizeof(num));
    if(nr != sizeof(num)){
        printf("Errore nella read da pipe con indice %d\n", N-1);
        exit(5);
    }
    
    printf("- Processo padre ha letto num = %d. FINE PIPELINE.\n", num);
